    ...
    <div class="upload-container">
        ...
        <div v-if="!notip" class="el-upload__tip">
            <div style="display: inline-block;">
                <el-alert :title="`上传图片支持 ${ ext.join(' / ') } 格式，单张图片大小不超过 ${ size }MB，建议图片尺寸为 ${w||width}*${h||height}，且图片数量不超过 ${ max } 张`" type="info" show-icon :closable="false" />
            </div>
        </div>
...
export default {
    ...
    methods: {
        // 预览
        preview(index) {
            this.dialogImageUrl = this.getImg(this.url[index])
        ...
        onError(res) {
            this.$message.warning(JSON.parse(res.message).message)
        }
    }
}
</script>
